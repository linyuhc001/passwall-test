...
# sed -i 's#net.ipv4.ip_forward=1#net.ipv4.ip_forward=0#g' package/base-files/files/etc/sysctl.d/10-default.conf #修改ipv4forward=0以适配xray的splice，主路由不适用
# sed -i 's#forwarding=1#forwarding=0#g' package/base-files/files/etc/sysctl.d/10-default.conf #修改ipv6forwarding=0以适配xray的splice，主路由不适用
